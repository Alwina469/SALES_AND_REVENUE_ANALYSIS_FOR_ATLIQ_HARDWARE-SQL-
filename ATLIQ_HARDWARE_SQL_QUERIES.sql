...
WITH UniqueProductCounts AS (
    ...
    JOIN fact_manufacturing_cost AS m ON p.product_code = m.product_code
    WHERE m.cost_year IN (2020, 2021)
...
WITH cte2 AS (
    SELECT 
        p.segment,
        COUNT(DISTINCT p.product_code) AS product_count,m.cost_year
    FROM dim_product AS p
    JOIN fact_manufacturing_cost AS m ON p.product_code = m.product_code
    WHERE m.cost_year IN (2020, 2021)
    GROUP BY p.segment, m.cost_year
),
segment_increase AS (
    SELECT y20.segment,
    ...
    FROM (SELECT segment, product_count FROM cte2 WHERE cost_year = 2020) AS y20
    JOIN (SELECT segment, product_count FROM cte2 WHERE cost_year = 2021) AS y21
    ON 
